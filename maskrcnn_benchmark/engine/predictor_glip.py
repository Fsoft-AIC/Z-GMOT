...
class GLIPDemo(object):
    ...
    def run_on_web_image(self,
        ...
        import time
        start = time.time()
        predictions, proposal_visual_features, proposal_fused_visual_features = self.compute_prediction(original_image, original_caption, custom_entity)
        end = time.time()
        print("Compute pred:", end-start)

        start = time.time()
        top_predictions, proposal_visual_features, proposal_fused_visual_features = self._post_process(predictions, proposal_visual_features, proposal_fused_visual_features, thresh)
        end = time.time()
        ...
        result, new_labels = self.overlay_entity_names(result, top_predictions)

        if self.cfg.MODEL.MASK_ON:
            result = self.overlay_mask(result, top_predictions)
        return result, top_predictions, proposal_visual_features, proposal_fused_visual_features # , new_labels
    ...
    def visualize_with_predictions(self, 
        ...
        if self.show_mask_heatmaps:
            return self.create_mask_montage(result, top_predictions)
        result = self.overlay_boxes(result, top_predictions, alpha=alpha, box_pixel=6)
    ...
    def compute_prediction(self, original_image, original_caption, custom_entity = None):
        ...
        # compute predictions
        with torch.no_grad():
            predictions, proposal_visual_features, proposal_fused_visual_features = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token)
        ...
        return prediction, proposal_visual_features, proposal_fused_visual_features
    ...
    def _post_process(self, predictions, proposal_visual_features, proposal_fused_visual_features,threshold=0.5):
        scores = predictions.get_field("scores")
        labels = predictions.get_field("labels").tolist()
        thresh = scores.clone()
        for i, lb in enumerate(labels):
        ...
        proposal_fused_visual_features = proposal_fused_visual_features[keep]

        scores = predictions.get_field("scores")
        _, idx = scores.sort(0, descending=True)
        return predictions[idx], proposal_visual_features[idx], proposal_fused_visual_features[idx]
    ...
    def overlay_boxes(self, image, predictions, alpha=0.5, box_pixel=3):
        ...
        for box, color in zip(boxes, colors):
            ...
            new_image = cv2.rectangle(
                new_image, tuple(top_left), tuple(bottom_right), (255, 0, 0), box_pixel) # tuple(color)
