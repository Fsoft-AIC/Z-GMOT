...
class GeneralizedVLRCNN(nn.Module):
    ...
    def forward(self, 
        ...
        if self.cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
            ...
            if captions is not None:
                ...
                if self.cfg.MODEL.LANGUAGE_BACKBONE.FREEZE:
                    ...
                    # flops, macs, params = calculate_flops(model=self.language_backbone, 
                    #                                     input_shape=(1,),
        ...
        if 'vl' in self.cfg.MODEL.SWINT.VERSION:
            ...
            # print(get_model_complexity_info(self.backbone, tuple(images.tensors.shape)[1:], as_strings=True, print_per_layer_stat=True))
            
            # from torch_flops import TorchFLOPsByFX  
            # flops_counter = TorchFLOPsByFX(self.backbone)
            # flops_counter.propagate(images.tensors)
            # result_table = flops_counter.print_result_table()       
            # total_flops = flops_counter.print_total_flops(show=True)  
            
            from fvcore.nn import FlopCountAnalysis
            flops = FlopCountAnalysis(self.backbone, images.tensors)
            flops.total()
            from calflops import calculate_flops
            flops, macs, params = calculate_flops(model=self.backbone, 
                                                input_shape=tuple(images.tensors.shape),
                                                output_as_string=True,
                                                output_precision=2)
            print("FLOPs:%s   MACs:%s   Params:%s \n" %(flops, macs, params))    
        ...
        # print('self.force_boxes', self.force_boxes)
        if self.force_boxes:
            proposals = []
            for t in targets:
            ...
            proposals, proposal_losses, fused_visual_features, proposal_visual_features = self.rpn(images, visual_features, targets, language_dict_features, positive_map,
                                              captions, swint_feature_c4) # VLDyHeadModule class
        ...
        assert proposal_visual_features.shape == fused_visual_features.shape
        return result, proposal_visual_features, fused_visual_features
