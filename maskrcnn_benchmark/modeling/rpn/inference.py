...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
        ...
        box_regression = permute_and_flatten(box_regression, N, A, 4, H, W)
        box_regression = box_regression.reshape(N, -1, 4)
        visual_features = permute_and_flatten(visual_features, N, A, F, H, W)
        visual_features = visual_features.reshape(N, -1, F)
        fused_visual_features = permute_and_flatten(fused_visual_features, N, A, F, H, W)
        fused_visual_features = fused_visual_features.reshape(N, -1, F)
        ...
        for per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors, per_visual_features, per_fused_visual_features \
                in zip(box_cls, box_regression, pre_nms_top_n, candidate_inds, anchors, visual_features, fused_visual_features):
            per_box_cls = per_box_cls[per_candidate_inds] 
            per_box_cls, top_k_indices = per_box_cls.topk(per_pre_nms_top_n, sorted=False)
            # import pdb; pdb.set_trace()
            per_candidate_nonzeros = per_candidate_inds.nonzero()[top_k_indices, :]

            # print('per_candidate_nonzeros', per_candidate_nonzeros)
            per_box_loc = per_candidate_nonzeros[:, 0]
            per_class = per_candidate_nonzeros[:, 1] + 1
            # import pdb; pdb.set_trace()
            ...
            selected_visual_features = per_visual_features[per_box_loc, :]
            selected_fused_visual_features = per_fused_visual_features[per_box_loc, :]

            boxlist = BoxList(detections, per_anchors.size, mode="xyxy")
            boxlist.add_field("labels", per_class)
            boxlist.add_field("scores", torch.sqrt(per_box_cls))
            boxlist = boxlist.clip_to_image(remove_empty=False)
            boxlist = remove_small_boxes(boxlist, self.min_size) # default self.min_size is 0, change it change features
            results.append(boxlist)
            # selected_visual_features.append(per_visual_features)
            # import pdb; pdb.set_trace()
        assert selected_visual_features.shape == selected_fused_visual_features.shape 
        return results, selected_visual_features, selected_fused_visual_features
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        sampled_boxes = []
        sampled_visual_features = []
        sampled_fused_visual_features = []
        anchors = list(zip(*anchors))
        for idx, (b, c, a, f, ff) in enumerate(zip(box_regression, centerness, anchors, visual_features, fused_visual_features)):
            o = None
            t = None
            d = None
            if box_cls is not None:
            ...
            boxes, selected_visual_features, selected_fused_visual_features = self.forward_for_single_feature_map(b, c, a, o, t, d, f, ff, positive_map)
            # import pdb;pdb.set_trace()
            sampled_boxes.append(boxes)
            sampled_visual_features.append(selected_visual_features)
            sampled_fused_visual_features.append(selected_fused_visual_features)
        # import pdb;pdb.set_trace()
        boxlists = list(zip(*sampled_boxes))
        boxlists = [cat_boxlist(boxlist) for boxlist in boxlists] # whether other sort ?
        # import pdb;pdb.set_trace()
        featurelists = torch.cat(sampled_visual_features, 0)
        fusedfeaturelists = torch.cat(sampled_visual_features, 0)

        if not (self.bbox_aug_enabled and not self.bbox_aug_vote):
            boxlists, featurelists, fusedfeaturelists = self.select_over_all_levels(boxlists, featurelists, fusedfeaturelists)

        return boxlists, featurelists, fusedfeaturelists

    # TODO very similar to filter_results from PostProcessor
    # but filter_results is per image
    # TODO Yang: solve this issue in the future. No good solution
    # right now.
    def select_over_all_levels(self, boxlists, featurelists, fusedfeaturelists):
        # import pdb; pdb.set_trace()
        num_images = len(boxlists)
        results = []
        for i in range(num_images):
            # multiclass nms
            # import pdb;pdb.set_trace()
            result, keep = boxlist_ml_nms(boxlists[i], self.nms_thresh)
            featurelists = featurelists[keep]
            fusedfeaturelists = fusedfeaturelists[keep]
            number_of_detections = len(result)
            ...
            results.append(result)
        # import pdb; pdb.set_trace()
        assert featurelists.shape == fusedfeaturelists.shape
        return results, featurelists, fusedfeaturelists
