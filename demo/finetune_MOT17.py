...
DATA_DIR = '/home/kimth1/MOT17/test'
SAVE_DIR = './results_MOT17/vis_before'
DROP_SAVE_DIR = './results_MOT17/vis_after'
BOX_SAVE_DIR = './results_MOT17/MOT17_boxes'
FINAL_VIS_DIR = './results_MOT17/vis_final'
...
running_videos = ['MOT17-08-SDP', 'MOT17-06-SDP' , 'MOT17-07-SDP', 'MOT17-12-SDP']
# 'MOT17-08-SDP' , 'MOT17-07-SDP
...
def get_object_in_image(glip_demo, caption, threshold, condition, frame_name, img_folder, save_vis_before, save_vis_after):
    ...
    result, pred_boxes, proposal_visual_features = glip_demo.run_on_web_image(img, caption, threshold)
    ...
    for box_id in range(bboxes.shape[0]):
        ...
        if w * h > img.shape[0] * img.shape[1] / 4: 
            continue
        if bboxes.shape[0] > 10 and condition and w * h > 5 * median_area:
            continue
    ...
    return bboxes[keep], proposal_visual_features[keep], scores[keep]
...
def main():
    ...
    for category in tqdm(os.listdir(DATA_DIR)):
        exist_video = os.listdir('/home/kimth1/GLIP/results_MOT17/MOT17_boxes')
        if f'{category}.txt' in exist_video:
            print(f'{category} is already')
            continue
        # if category not in running_videos:
        #     continue
        ...
        if CAPTION_MAP.get(category) is not None:
            ...
            caption, threshold, condition = "one human . car", 0.41, False
        try:
            caption2, threshold2, condition2 = CAPTION_MAP2[category], THRES_MAP2[category], COND_MAP2[category]
        except:
            caption2, threshold2, condition2 = caption, threshold, condition
        ...
        for frame_id, frame_name in enumerate(choice_fnames, 1):
            # get features
            high_visual_bboxes, high_visual_features, high_scores = get_object_in_image(glip_demo, caption, threshold, condition, frame_name, img_folder, save_vis_before, save_vis_after)            
            if high_visual_bboxes.shape[0] == 0:
                high_visual_bboxes, high_visual_features, high_scores = get_object_in_image(glip_demo, caption, threshold / 2, condition, frame_name, img_folder, save_vis_before, save_vis_after) 
            if caption != caption2:
                low_visual_bboxes, low_visual_features, low_scores = get_object_in_image(glip_demo, caption2, threshold2, condition2, frame_name, img_folder, save_vis_before2, save_vis_after2)  
            else:
