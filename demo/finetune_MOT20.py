...
DATA_DIR = '/home/kimth1/MOT20/test'
SAVE_DIR = './results_MOT20/vis_before'
DROP_SAVE_DIR = './results_MOT20/vis_after'
BOX_SAVE_DIR = './results_MOT20/boxes'
FINAL_VIS_DIR = './results_MOT20/vis_final'
...
run_videos = ['MOT20-06', 'MOT20-07', 'MOT20-08']
...
def get_object_in_image(glip_demo, caption, threshold, condition, frame_name, img_folder, save_vis_before, save_vis_after):
    ...
    for box_id in range(bboxes.shape[0]):
        ...
        if w * h > img.shape[0] * img.shape[1] / 4: 
            continue
        if bboxes.shape[0] > 10 and condition and w * h > 6 * median_area:
            #print('debug')
            #import pdb; pdb.set_trace()
            continue
...
def main():
    ...
    for category in tqdm(os.listdir(DATA_DIR)):
        ...
        if CAPTION_MAP.get(category) is not None:
            ...
            caption, threshold, condition = "one human . car", 0.38, False
        try:
