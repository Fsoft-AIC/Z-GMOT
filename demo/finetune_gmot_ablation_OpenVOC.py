...
version = 3
SAVE_DIR = f'./OpenVOC/ablation_{version}/vis_before'
DROP_SAVE_DIR = f'./OpenVOC/ablation_{version}/vis_after'
BOX_SAVE_DIR = f'./OpenVOC/ablation_{version}/GMOT_40_boxes'
FINAL_VIS_DIR = f'./OpenVOC/ablation_{version}/vis_final'
...
CAPTION_MAP = {
    # 'person-1': 'black human',
    #'person-2': 'green uniform' 
    # 'person-2': 'green uniform'
    # 'car-3': 'yellow car'
    'stock-3': 'cow . gaur . buffalo'
    # 'stock-3': 'wolf'
}
THRES_MAP = {
    #'person-1': 0.43,
...
def main():
    ...
    for category in tqdm(os.listdir(DATA_DIR)):
        ...
        save_vis_before = osp.join(SAVE_DIR, category + f'-high-{THRES_MAP[category]}')
        save_vis_after = osp.join(DROP_SAVE_DIR, category + f'-high-{THRES_MAP[category]}')
        save_vis_before2 = osp.join(SAVE_DIR, category + f'-low-{THRES_MAP2[category]}')
        save_vis_after2 = osp.join(DROP_SAVE_DIR, category + f'-low-{THRES_MAP2[category]}')
